  ...
  return (
    ...
    >
      <TiltCard
        ...
        className={`relative group bg-graphite/25 backdrop-blur-xl rounded-3xl p-10 border border-white/5 hover:border-${pillar.color === 'blue' ? 'accent-blue/30' : 'accent-cyan/30'} flex flex-col justify-between overflow-hidden cursor-pointer h-full min-h-[400px]`}
      >
        ...
        <div className={`absolute -top-20 -right-20 w-64 h-64 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 ${pillar.color === 'blue' ? 'bg-accent-blue/10' : 'bg-accent-cyan/10'}`} />
        ...
        <div style={{ transform: "translateZ(60px)" }} className="relative z-10 flex flex-col h-full pointer-events-none drop-shadow-2xl">
          <div className="mb-auto">
            ...
            <div className={`text-sm font-bold uppercase tracking-widest ${pillar.color === 'blue' ? 'text-accent-blue' : 'text-accent-cyan'} drop-shadow-[0_2px_10px_rgba(0,0,0,0.9)]`}>
              {pillar.unit}
            </div>
          </div>
