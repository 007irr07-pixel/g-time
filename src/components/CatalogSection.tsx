...
function CatalogCard({ item, index }: { item: CatalogItem; index: number }) {
  ...
  return (
    <motion.div variants={itemVariants} className={index >= 3 ? "lg:col-span-3" : "lg:col-span-2"}>
      <TiltCard
        className={`group relative h-full w-full min-h-[420px] bg-gradient-to-br from-silver/5 to-graphite/10 backdrop-blur-xl rounded-3xl p-8 sm:p-10 cursor-pointer overflow-hidden transition-all duration-500 border border-silver/20 ${item.borderColor}`}
